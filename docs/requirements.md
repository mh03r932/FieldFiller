...
| FR-002 | Fill Current Form        | As a Tester, I want to fill only the form I am pointing at — resolved by the DD-008 ladder where the page declares no `<form>`, and widened to the page only when I have pointed at nothing — so that I do not disturb other forms on the same page, and never watch a keypress do nothing. | High | Open |
...
| FR-005 | Keyboard Trigger         | As a Tester, I want at least one fill scope bound to a shipped keyboard shortcut, and every other scope assignable through the browser's own shortcuts settings, so that I can fill without leaving the keyboard and can bind the scopes I use to keys that suit me. | High | Open |
...
| FR-078 | Honest Cascade Bounds    | As a Tester, I want a fill that cannot settle to stop at a declared limit and tell me it stopped, naming how many fields may be stale, so that a page which fights back produces a bounded honest result rather than a hang or a clean-looking lie. | High | Done — a frame reports its pass count, its cap reason and how many controls a further pass would have covered; the surface that carries it to the user is DD-006's to decide |
| FR-079 | User Input Precedence    | As a Tester, I want the extension to stop writing to any control I have touched since the fill began, and to abandon the rest of a cascade the moment I start typing, so that a fill lasting seconds can never overwrite what I am doing. | High | Done — discriminated by `isTrusted`, which only the platform can set, so it cannot be confused with the events the extension dispatches itself |
...
| NFR-033 | Network Absence Verification | Absence of network capability must be established by static analysis of the built bundle in CI — no `fetch`, `XMLHttpRequest`, `WebSocket`, `EventSource` or `sendBeacon` reachable in shipped code, and no external URL in any shipped HTML, CSS, or the manifest. Runtime traffic monitoring in Chromium runs in addition, never instead: it can only demonstrate the code paths it exercises, and it cannot observe requests originating in the background context without browser-level instrumentation. | Security | High | Open |
