...
| FR-076 | Write Verification       | As a Tester, I want every write checked against the control after it is made and again when the fill ends, judged by what the control kind can accept rather than by exact string equality, so that a value the page rejected or reverted is reported as failed instead of counted as filled. | High | Open |
