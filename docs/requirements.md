...
| FR-009 | Fill Result Report       | As a Tester, I want every fill to return a structured report of each field's outcome — filled, skipped with reason, or failed with cause — so that I can tell a no-op from a failure and the behaviour is assertable in a test. | High | Open |
...
| FR-018 | Select Elements           | As a Tester, I want a random enabled option chosen for single selects and a random enabled subset for multi-selects, skipping an empty placeholder option, so that every dropdown ends in a valid submittable state. | High | Open |
...
| FR-024 | Confirmation Mirroring    | As a Tester, I want fields matching my confirmation keywords to repeat the previous value so that "confirm email" and "retype password" match.| High     | Open   |
...
| FR-067 | Source-Scoped Rules       | As a Power User, I want to restrict an individual rule to specific matching sources so that a noisy `class` attribute cannot trigger a rule meant for `name`. | Medium | Open |
...
| FR-071 | Honeypot Avoidance        | As a Tester, I want fields hidden by any technique — zero opacity, off-screen placement, clipping — treated as hidden so that anti-bot honeypots are never filled. | High | Open |
...
| NFR-006 | Zero Network               | The extension must issue zero outbound network requests in all code paths; verified by an automated test asserting no requests over a full session. | Security | High | Open |
...
| NFR-014 | End-to-End Coverage        | An automated browser test must fill a reference page covering every supported control type in both Chromium and Firefox.        | Maintainability | High     | Open   |
...
| C-008 | No Backend                   | No server-side component, account system, or third-party SDK may be introduced (per PD-001).                                      | Technical   | High     | Open   |
