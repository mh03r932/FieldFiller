...
| FR-081 | Custom Combobox Filling  | As a Tester, I want ARIA comboboxes and listboxes filled through the keyboard and click interactions a user would use, with the page's state restored and the control reported as skipped when they do not work, so that design-system dropdowns are covered where possible and never left in a broken open state. | Medium | Open |
...
| **FR-081 vs NFR-003** | Detecting and driving custom comboboxes is agent-side code, and detection also widens the walk's candidate selector on every page — spending latency budget everywhere to serve a minority of pages. | Open, but the tiebreak is already set: per `docs/vision.md` §3, reaching further is coverage rather than correctness, so NFR-003 wins and FR-081 is the part of DD-009 that gets cut if the budget binds. A skipped combobox reported with its reason is a correct outcome. Gated on measuring candidate-set inflation against the reference page before implementation, not during it. |
