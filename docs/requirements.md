...
| FR-076 | Write Verification       | As a Tester, I want every write checked against the control after it is made and again when the fill ends, judged by what the control kind can accept rather than by exact string equality, so that a value the page rejected or reverted is reported as failed instead of counted as filled. | High | Partial — the check after each write landed 2026-08-15; the check when the fill ends needs the loop (DD-009 step B), because there is no "end" to check at until then |
| FR-077 | Cascade Resolution       | As a Tester, I want fields whose options or availability depend on an earlier answer filled after that answer takes effect — options rewritten, fields revealed, controls enabled — so that a country/state/city form ends complete instead of half-filled from a stale snapshot. | High | Open |
| FR-078 | Honest Cascade Bounds    | As a Tester, I want a fill that cannot settle to stop at a declared limit and tell me it stopped, naming how many fields may be stale, so that a page which fights back produces a bounded honest result rather than a hang or a clean-looking lie. | High | Open |
| FR-079 | User Input Precedence    | As a Tester, I want the extension to stop writing to any control I have touched since the fill began, and to abandon the rest of a cascade the moment I start typing, so that a fill lasting seconds can never overwrite what I am doing. | High | Open |
...
| FR-080 | Stable Values Within a Fill | As a Tester, I want a control that has to be filled more than once during a single fill — because the page wiped it or replaced it — to receive the same value each time, so that a field and its confirmation cannot drift apart when the page makes us write twice. | High | Open |
